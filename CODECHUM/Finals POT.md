...
void heapify(MinHeap* pq, int i) {
 ...
 void heapify(MinHeap* pq, int i) {
    while (1) {
        ...
        // If parent is already smallest → heap property satisfied → STOP
        if (smallest == i) {
            break;
        }
  ...
  //  int parent = (i - 1 ) / 2;
    ...
    // if(left < pq->size && pq->heap[left].duration < 
    //    pq->heap[smallest].duration){
    //    smallest = left;
    // }

    // if(right < pq->size && pq->heap[right].duration < 
    //    pq->heap[smallest].duration){
    //    smallest = right;
    // }
...
# 🧠 How it works (non-recursive)

Start at index i

Compare parent with left & right children

If the parent is already the smallest → stop

Otherwise swap parent with smallest child

Move i down to the swapped child
