...
void genReceipt(Cart* C, PromoCode promoList[], char promoCode[]){
	int x;
	for (x = 0 ; x < PMSIZE && strcmp(promoCode, promoList[x].promoCode) != 0 ; x++){};     // Look for the promo code in the promoList array
	if (x < PMSIZE){ //     // If a valid promo code was found (index is within bounds):
		SList trav;
		float totalPrice = 0;

    
        // Traverse the shopping cart linked list (sCart)
        // For each item in the cart, compute line item total (unitPrice * qty)
        // and accumulate to totalPrice.
		for (trav = C->sCart ; trav != NULL ; trav = trav->nextItem){
			C->receipt.totalPrice = totalPrice += (trav->item.unitPrice * trav->item.qty);
		}
            // Store the computed total price inside the receipt
